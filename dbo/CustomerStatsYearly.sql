CREATE VIEW dbo.CustomerStatsYearly
AS
SELECT dbo.Orders.CustomerId, YEAR(dbo.Orders.OrderDate) AS year, ROUND(SUM(dbo.OrderDetails.Quantity * ISNULL(dbo.Menu.UnitPrice, dbo.Products.DefaultPrice) - dbo.OrderDetails.DiscountAmount), 2) AS totalPrice,
...
GROUP BY dbo.Orders.CustomerId, YEAR(dbo.Orders.OrderDate)
go
...
Begin DesignProperties = 
   ...
   Begin CriteriaPane = 
      Begin ColumnWidths = 12
         ...
         Or = 1350
         Or = 1350
         Or', 'SCHEMA', 'dbo', 'VIEW', 'CustomerStatsYearly'
go
...
exec sp_addextendedproperty 'MS_DiagramPane2', N' = 1350
   ...
   End
End
', 'SCHEMA', 'dbo', 'VIEW', 'CustomerStatsYearly'
go

exec sp_addextendedproperty 'MS_DiagramPaneCount', 2, 'SCHEMA', 'dbo', 'VIEW', 'CustomerStatsYearly'
go

grant select on CustomerStatsYearly to manager
go
